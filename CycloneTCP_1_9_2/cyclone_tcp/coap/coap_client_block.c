...
error_t coapClientSetTxBlockSize(CoapClientRequest *request, uint32_t blockSize)
{
...
error_t coapClientSetRxBlockSize(CoapClientRequest *request, uint32_t blockSize)
{
   //Make sure the CoAP request handle is valid
   if(request == NULL)
      return ERROR_INVALID_PARAMETER;
