...
struct _SmtpClientContext
{
   ...
   uint32_t replyCode;                           ///<SMTP reply code
   uint32_t recipientIndex;                      ///<Index of the current recipient
};


//Callback function to parse a response line
typedef error_t (*SmtpReplyCallback)(SmtpClientContext *context, char_t *replyLine, uint32_t replyCode);
...
error_t smtpClientWriteMailHeader(SmtpClientContext *context,
   ...
   uint32_t numRecipients, const char_t *subject);
...
uint32_t smtpClientGetReplyCode(SmtpClientContext *context);
