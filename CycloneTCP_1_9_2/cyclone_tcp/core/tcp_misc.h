...
void tcpFastRecovery(Socket *socket, TcpHeader *segment, uint32_t n);
...
error_t tcpNagleAlgo(Socket *socket, uint32_t flags);

void tcpChangeState(Socket *socket, TcpState newState);

void tcpUpdateEvents(Socket *socket);
uint32_t tcpWaitForEvents(Socket *socket, uint32_t eventMask, systime_t timeout);
