...
//TCP specific variables
   ...
   uint32_t dupAckCount;            ///<Number of consecutive duplicate ACKs
   ...
   uint32_t retransmitCount;        ///<Number of retransmissions

   TcpSynQueueItem *synQueue;     ///<SYN queue for listening sockets
   uint32_t synQueueSize;           ///<Maximum number of pending connections for listening sockets

   uint32_t wndProbeCount;          ///<Zero window probe counter
   ...
   uint32_t sackBlockCount;                       ///<Number of non-contiguous blocks that have been received
#endif
...
typedef struct
{
   ...
   uint32_t eventMask;  ///<Requested events
   uint32_t eventFlags; ///<Returned events
} SocketEventDesc;
...
Socket *socketOpen(uint32_t type, uint32_t protocol);
...
error_t socketShutdown(Socket *socket, uint32_t how);
void socketClose(Socket *socket);

error_t socketPoll(SocketEventDesc *eventDesc, uint32_t size, OsEvent *extEvent, systime_t timeout);
error_t socketRegisterEvents(Socket *socket, OsEvent *event, uint32_t eventMask);
error_t socketUnregisterEvents(Socket *socket);
error_t socketGetEvents(Socket *socket, uint32_t *eventFlags);

error_t getHostByName(NetInterface *interface,
   const char_t *name, IpAddr *ipAddr, uint32_t flags);
