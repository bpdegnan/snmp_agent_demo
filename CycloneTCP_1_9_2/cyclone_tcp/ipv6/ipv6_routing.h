...
typedef struct
{
   ...
   uint32_t prefixLen;        ///<IPv6 prefix length
...
error_t ipv6AddRoute(const Ipv6Addr *prefix, uint32_t prefixLen,
   NetInterface *interface, const Ipv6Addr *nextHop, uint32_t metric);

error_t ipv6DeleteRoute(const Ipv6Addr *prefix, uint32_t prefixLen);
