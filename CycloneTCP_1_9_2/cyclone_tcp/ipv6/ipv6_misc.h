...
error_t ipv6SetAddr(NetInterface *interface, uint32_t index,
...
void ipv6AddPrefix(NetInterface *interface, const Ipv6Addr *prefix,
   uint32_t length, bool_t onLinkFlag, bool_t autonomousFlag,
   uint32_t validLifetime, uint32_t preferredLifetime);
...
uint32_t ipv6GetAddrScope(const Ipv6Addr *ipAddr);
uint32_t ipv6GetMulticastAddrScope(const Ipv6Addr *ipAddr);

uint32_t ipv6GetCommonPrefixLength(const Ipv6Addr *ipAddr1,
