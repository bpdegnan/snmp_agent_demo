...
typedef struct
{
   uint32_t version;                                           ///<HTTP version number
...
{
   uint32_t version;              ///<HTTP version number
   uint32_t statusCode;           ///<HTTP status code
   bool_t keepAlive;
   bool_t noCache;
   uint32_t maxAge;
...
typedef struct
{
   ...
   uint32_t backlog;                                              ///<Maximum length of the pending connection queue
   uint32_t maxConnections;                                       ///<Maximum number of simultaneous connections
...
error_t httpSendRedirectResponse(HttpConnection *connection,
   uint32_t statusCode, const char_t *uri);
