...
error_t ssiExecuteScript(HttpConnection *connection, const char_t *uri, uint32_t level)
{
   error_t error;
   size_t length;
...
error_t ssiProcessCommand(HttpConnection *connection,
   const char_t *tag, size_t length, const char_t *uri, uint32_t level)
{
   error_t error;
...
error_t ssiSearchTag(const char_t *s, size_t sLen, const char_t *tag, size_t tagLen, uint32_t *pos)
{
   uint32_t i;
   uint32_t j;
