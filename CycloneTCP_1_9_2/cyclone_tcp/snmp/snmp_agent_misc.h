...
error_t snmpWriteTrapVarBindingList(SnmpAgentContext *context,
   uint32_t genericTrapType, uint32_t specificTrapCode,
   const SnmpTrapObject *objectList, uint32_t objectListSize);

error_t snmpTranslateStatusCode(SnmpMessage *message, error_t status, uint32_t index);
