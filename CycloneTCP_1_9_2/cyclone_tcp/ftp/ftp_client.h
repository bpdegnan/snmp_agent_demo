...
struct _FtpClientContext
{
   ...
   uint32_t replyCode;                         ///<FTP reply code
};
...
error_t ftpClientConnect(FtpClientContext *context,
   const IpAddr *serverIpAddr, uint16_t serverPort, uint32_t mode);
...
error_t ftpClientReadFile(FtpClientContext *context, void *data, size_t size,
   size_t *received, uint32_t flags);
...
uint32_t ftpClientGetReplyCode(FtpClientContext *context);
