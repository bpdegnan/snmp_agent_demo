...
error_t resOpenFile(FsFile *file, const DirEntry *dirEntry, uint32_t mode)
{
   file->mode = mode;
   file->offset = 0;
...
uint32_t resReadFile(FsFile *file, void *data, size_t length)
{
...
uint32_t fileGetSize(FILE *stream)
{
   uint32_t n;
