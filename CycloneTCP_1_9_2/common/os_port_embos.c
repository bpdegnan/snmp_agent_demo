...
bool_t osCreateStaticTask(OsTask *task, const char_t *name, OsTaskCode taskCode,
   void *param, void *stack, size_t stackSize, int32_t priority)
{
   //Create a new task
   OS_CreateTaskEx(task, name, priority, taskCode,
      stack, stackSize * sizeof(uint32_t), 1, param);
...
OsTask *osCreateTask(const char_t *name, OsTaskCode taskCode,
   void *param, size_t stackSize, int32_t priority)
{
   uint32_t i;
   OS_TASK *task;
   void *stack;
   ...
   if(i < OS_PORT_MAX_TASKS)
   {
      ...
      if(task != NULL)
      {
         ...
         stack = osAllocMem(stackSize * sizeof(uint32_t));
...
bool_t osCreateSemaphore(OsSemaphore *semaphore, uint32_t count)
{
   //Create a semaphore
